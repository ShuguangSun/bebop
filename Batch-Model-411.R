...
# Scenario 1 ----
scenario <- 1
json_file <- paste0('scenario', scenario, '.json')
...
sims <- run_batch(seed, 100, get_data_args, priors, model, 
                  sampling_args, refine_args, label, out_file)
...
sims <- run_batch(seed, 1000, get_data_args, priors, model, 
                  sampling_args, refine_args, label, out_file)

scenario <- '7-equal-cohorts'
json_file <- paste0('scenario', scenario, '.json')
out_file <- paste0(out_dir, json_file)
label <- paste0('Scenario ', scenario)
get_data_args <- list(
