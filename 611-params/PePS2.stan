...
functions {
  real log_joint_pdf(int num_patients, int[] eff, int[] tox, int[] x1, int[] x2, int[] x3,
                     real alpha, real beta, real gamma, real zeta, real eta, 
                     real kappa, real lambda, real psi) {
    real p;
    p = 0;
...
data {
  int<lower=0> num_patients; // number of obs
  int<lower=0, upper=1> eff[num_patients]; // event one
  int<lower=0, upper=1> tox[num_patients]; // event two
  int<lower=0, upper=1> x1[num_patients];  // x1=1 if pre-treated, else 0
  int<lower=0, upper=1> x2[num_patients];  // x2=1 if PD-L1 is low
  int<lower=0, upper=1> x3[num_patients];  // x3=1 if PD-L1 is medium
...
transformed parameters {
  ...
  prob_eff[1] = inv_logit(alpha + beta*0 + gamma*1 + zeta*0 + eta*0*1 + kappa*0*0);
  prob_eff[2] = inv_logit(alpha + beta*0 + gamma*0 + zeta*1 + eta*0*0 + kappa*0*1);
  prob_eff[3] = inv_logit(alpha + beta*0 + gamma*0 + zeta*0 + eta*0*0 + kappa*0*0);
  prob_eff[4] = inv_logit(alpha + beta*1 + gamma*1 + zeta*0 + eta*1*1 + kappa*1*0);
  prob_eff[5] = inv_logit(alpha + beta*1 + gamma*0 + zeta*1 + eta*1*0 + kappa*1*1);
  prob_eff[6] = inv_logit(alpha + beta*1 + gamma*0 + zeta*0 + eta*0*0 + kappa*0*0);
...
model {
  ...
  target += normal_lpdf(kappa | kappa_mean, kappa_sd);
  target += normal_lpdf(lambda | lambda_mean, lambda_sd);
  target += normal_lpdf(psi | psi_mean, psi_sd);
  target += log_joint_pdf(num_patients, eff, tox, x1, x2, x3, alpha, beta, gamma, 
                          zeta, eta, kappa, lambda, psi);
}
...
generated quantities {
  ...
  for (j in 1:num_patients) { 
    ...
    prob1 = inv_logit(alpha + beta * x1[j] + gamma * x2[j] + zeta * x3[j] + 
                      eta*x1[j] * x2[j] + kappa*x1[j] * x3[j]);
    prob2 = inv_logit(lambda);
    p_j = prob1^eff[j] * (1-prob1)^(1-eff[j]) * prob2^tox[j] * (1-prob2)^(1-tox[j]) +
      (-1)^(eff[j]+tox[j]) * prob1 * prob2 * (1-prob1) * (1-prob2) * (exp(psi)-1) / (exp(psi)+1);
    log_lik[j] = log(p_j);
  }
}
